...
The browser binary is fetched with **only your Capium license key** — no separate token, no
manual download, no per-platform URL to figure out. Set the key once:
...
Then the right build for your OS/arch downloads automatically — either **on first launch**, or
ahead of time with the CLI:

```bash
npx capiumbrowser install     # ...or skip it: the first launch() downloads the binary for you.
```

That same key both **authorizes the download** and **licenses the browser**, and it's passed to
the binary through the environment — never on argv. The download is checksum-verified before
anything runs and cached, so it's fetched once and reused. Run `npx capiumbrowser` (the `info`
